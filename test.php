...
<html>
   ...
   <body>
      <?php
            ...
            $sql = "SELECT Distance FROM users WHERE user_id='$User';";
			mysqli_select_db( $conn,'test_db');
            $retval =  mysqli_query( $conn , $sql);
            if ($retval->num_rows > 0) {
				$_SESSION['authuser']  = 1;
				while($row = mysqli_fetch_array($retval, MYSQL_ASSOC))
			{ $r_distance= $row['Distance'];}}
            
			
            
			$sql = "SELECT * FROM users WHERE donation_amount>0 and ($r_distance-Distance BETWEEN -15 and 15);";
